...
export default {
  ...
  methods: {
    drawLine() {
      ...
      var maxy = Math.max(...this.data.zongshuA) * 1.5 //y轴最大值
      var xarr = this.data.xFontcolor
      ...
      var num1 = this.data.Xquantityelectricity
            ...
            markLine: {
              ...
              data: [
                // 要求调节速率
                {
                  ...
                  yAxis: num2, // 警戒线的标注值，可以有多个yAxis,多条警示线 或者采用 {type : 'average', name: '平均值'}，type值有 max min average，分为最大，最小，平均值
                },
      ...
      this.data?.yxdates?.forEach((v) => {
        option.series[0].markLine.data.push({
          ...
          // 这里要返还警戒线的索引  我才能根据索引去改变之前的蓝色
          // xAxis: '16:45', // 警戒线的标注值，可以有多个yAxis,多条警示线 或者采用 {type : 'average', name: '平均值'}，type值有 max min average，分为最大，最小，平均值
          name: `${String(v[0])}万kW·h`,
          xAxis: v[0], // 警戒线的标注值，可以有多个yAxis,多条警示线 或者采用 {type : 'average', name: '平均值'}，type值有 max min average，分为最大，最小，平均值
        })
      })
      // 公司
      this.data?.arr?.forEach((v, i) => {
        option.series[0].markLine.data.push({
          silent: true, //鼠标悬停事件 true没有，false有
          lineStyle: {
            width: 0,
          ...
          // xAxis: '16:45', // 警戒线的标注值，可以有多个yAxis,多条警示线 或者采用 {type : 'average', name: '平均值'}，type值有 max min average，分为最大，最小，平均值
          name: this.data.xs[i],
          xAxis: v, // 警戒线的标注值，可以有多个yAxis,多条警示线 或者采用 {type : 'average', name: '平均值'}，type值有 max min average，分为最大，最小，平均值
        })
      })
      // 要求调节电量
      option.series[0].markLine.data.push({
        ...
        // xAxis: '16:45', // 警戒线的标注值，可以有多个yAxis,多条警示线 或者采用 {type : 'average', name: '平均值'}，type值有 max min average，分为最大，最小，平均值
        // xAxis: this.data.Xquantityelectricity, // 警戒线的标注值，可以有多个yAxis,多条警示线 或者采用 {type : 'average', name: '平均值'}，type值有 max min average，分为最大，最小，平均值
        xAxis: num1, // 警戒线的标注值，可以有多个yAxis,多条警示线 或者采用 {type : 'average', name: '平均值'}，type值有 max min average，分为最大，最小，平均值
      })
