<template>
  <div class="password">
    <div class="content">
      <h2 class="title">{{obj['msg.modify.password.title'] || '重置密码'}}</h2>
      <!-- 密码表单 -->
      <el-form
        :model="form"
        :rules="rules"
      ...
      >
      <!-- 新密码 -->
        <el-form-item :label="obj['msg.modify.password.newPassword'] || '新密码'" prop="newPassword">
          ...
          >
            <div>
              <h4 class="tit">{{obj['msg.modify.password.passwordMustMeet']}}:</h4>
              <div class="p_dialog" v-if="text.length > 0">
                <!-- 改用后端顶的校验贵则 -->
                <div class="item" v-for="(item,index) in text"><i :class="{'active':rulesAll[index][index]}" class="el-icon-circle-check"></i><span class="m_left" :class="{'active':rulesAll[index][index]}">{{item.i18nDesc}}</span></div>
              </div>
              <h4 class="tit">{{obj['msg.modify.password.passwordStrength']}}:</h4>
              <div class="strength">
                <div class="weak" :class="{'weakactive':weakFlag}">{{obj['msg.modify.password.weak']}}</div>
                <div class="centre" :class="{'centreactive':centreFlag}">{{obj['msg.modify.password.centre']}}</div>
                <div class="strong" :class="{'strongactive':strongFlag}">{{obj['msg.modify.password.strong']}}</div>
              </div>
            </div>
          </el-popover>
        </el-form-item>
        <!-- 确认密码 -->
        <el-form-item :label="obj['msg.modify.password.confirmPassword'] || '确认密码'" prop="confirmPassword">
          <el-input v-model.trim="form.confirmPassword"></el-input>
        </el-form-item>
      </el-form>
      <div class="btn" @click="save">{{obj['msg.modify.password.set_up'] || '设置'}}</div>
    </div>
  </div>
</template>
  ...
  data() {
    var checkPassword = (rule, value, callback) =>{
      if(value != this.form.newPassword){
       return callback(new Error(this.obj['msg.modify.password.p_TwoPasswordsConsistent']))
      }
      callback()
    }
    return {
  ...
  methods: {
    ...
    initRules() {
      this.rules.newPassword = [
        { required: true, message: this.obj['msg.modify.password.p_newPassword'], trigger: "blur" },
      ];
      this.rules.confirmPassword.push({ required: true, message: this.obj['msg.modify.password.p_confirmPassword'], trigger: "blur" })
    },
    // 请求国际化数据
    getI8Data(language,proxy){
      ...
      axios.post(`http://10.13.77.218:8080/uaa/queryI18nValue`, params).then(function (response) {
    ...
    getRules(formRules){
      let that = this
      axios.post(`http://10.13.77.218:8080/uaa/passwordStrengthRules`, {}).then( function (response) {
