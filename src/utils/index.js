 ...
 * */
export function checkFileFn(fileMd5, totalChunks) {
  return new Promise((resolve, reject) => {
    // resolve(axios.post(`http://127.0.0.1:8686/bigfile/check?fileMd5=${fileMd5}`))
    resolve(
      axios.get(
        `/api/web-server/uc-api/file/check?fileId=${fileMd5}&totalChunks=${totalChunks}`
      )
    )
  })
}
...
export function sliceFileUploadFn(formData) {
  return new Promise((resolve, reject) => {
    ...
    resolve(axios.post(`/api//web-server/uc-api/file/upload`, formData))
  })
}
 ...
 * 告知后端要去合并前端上传的文件了
 * */
export function tellBackendMergeFn(formData) {
  return new Promise((resolve, reject) => {
    // resolve(axios.post(`http://127.0.0.1:8686/bigfile/merge?fileName=${fileName}&fileMd5=${fileMd5}`))
    resolve(axios.post(`/api/web-server/uc-api/file/merge `, formData))
  })
}

//下载接口
export function download(id = 1833) {
  return new Promise((resolve, reject) => {
    resolve(
      axios.get(`/api/web-server/uc-api/file/download?id=${id}`, {
