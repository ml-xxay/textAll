    ...
    <div class="left" ref="leftRef">
      ...
      >
        <el-form-item
          ...
          :label="item.label"
          :prop="item.prop"
          v-for="(item, index) in configLeft.formItems"
          :key="index"
        >
          <!-- 配置文件类型是 input 框-->
          ...
          <!-- 配置文件是 file  文件-->
          <template v-else-if="item.type == 'file'"> 文件展示 </template>
          <!-- 配置文件是 textarea  文件-->
      ...
      <!-- 操作按钮 -->
      <div class="footer">
        <el-button type="primary" @click="isDisabledLeft = !isDisabledLeft">{{
          isDisabledLeft ? '编辑' : '保存'
        }}</el-button>
      </div>
    </div>
    <!-- 右侧tabs -->
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        >
          <template #default>
            <el-form
              :model="pane.data"
              :ref="`rightForm-${pane.id}`"
              :rules="configLeft.rules"
              class="form-left-box right-form"
              label-width="120px"
              label-position="top"
              :disabled="pane.id !== currentActiveTab || isDisabledRight"
            >
              <el-form-item
                class="item"
                :label="item.label"
...
export default {
      ...
      configLeft: {
        ...
        rules: {
          ...
          certificateReportNumber: [
            { required: true, message: '请输入证书/报告号', trigger: 'blur' },
          ],
          standard: [
  ...
  mounted() {
    ...
    setTimeout(() => {
      ...
      const allFormRefs = Object.keys(this.$refs).filter((ref) =>
        ref.startsWith('rightForm-')
      )
      allFormRefs.forEach((formRef) => {
        this.$refs[formRef][0].$el.addEventListener('scroll', () =>
          this.handleRightScroll(formRef)
        )
      })
    }, 5000)
  ...
  methods: {
    ...
    toggleEdit() {
      ...
      if (this.isDisabledRight) {
        console.log('我需要请求税局')
        const currentForm = this.panes.find(
          (pane) => pane.id === this.currentActiveTab
        )
        ...
        console.log(
          this.$refs[`rightForm-${this.currentActiveTab}`],
          '---我是要提交表单的dom----'
        )

        this.$refs[`rightForm-${this.currentActiveTab}`][0].validate(
          (valid) => {
            if (!valid) {
    ...
    updateProductLineDropdown(tabId, modelIds) {
      ...
      const modelItem = this.configLeft.formItems.find(
        (item) => item.prop === 'productModel'
      ) // 从配置里面找出产品型号这一项
      if (
        modelItem &&
        modelItem.type === 'multiple' &&
        modelItem.prop == 'productModel'
      ) {
        let selectedModels = currentForm.data.productModel
        let combinedProductLines = []

        selectedModels.forEach((modelId) => {
          const productLinesForModel = this.dictionaries.productModel.find(
            (p) => p.id === modelId
          ).productLine
          combinedProductLines =
            combinedProductLines.concat(productLinesForModel)
        })
        ...
        currentForm.data.productLine = uniqueProductLines.map(
          (option) => option.id
        )
      }
    },
  ...
  beforeDestroy() {
    ...
    allFormRefs.forEach((formRef) => {
      this.$refs[formRef][0].$el.removeEventListener('scroll', () =>
        this.handleRightScroll(formRef)
      )
    })
  },
